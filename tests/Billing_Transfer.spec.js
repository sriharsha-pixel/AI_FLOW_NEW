...
test("Mark as Billing Transfer", async ({ page }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
        ...
        const transactionDetails = await cashPosting.markAsBillingTransfer();
        if (transactionDetails) {
            console.log(`Marked as Billing Transfer`);
            return;
        }
    }
})
...
test("Unmark as Billing Transfer", async ({ page }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
        ...
        const transactionDetails = await cashPosting.unmarkAsBillingTransfer();
        if (transactionDetails) {
            console.log(`Unmarked as Billing Transfer`);
            return;
        }
    }
})
...
test("Bulk Mark as Billing Transfer", async ({ page }) => {
    ...
    for (let i = 0; i < cardCount; i++) {
        ...
        const numOfTransactions = await cashPosting.bulkMarkAsBillingTransfer();
        if (numOfTransactions) {
            console.log(`Bulk marked ${numOfTransactions} transactions as Billing Transfer`);
            return;
        }
    }
})